...
Gantt 上でも dependency line を表示する。

依存関係の保存先は未決（§26）。Projects v2 のテキストフィールドに
`blocked-by: #101` の形式で持つ案と、GitHub 標準の sub-issue 関係を使う案がある。

### 15.2 自動スケジューリング

依存関係と期間からスケジュールを自動生成する。
...
この機能は1操作で多数のタスクを書き換えるため、
ミューテーションキュー（§16）が確立してから着手する。
...
| Step | 内容 |
|---|---|
| 11 | 依存関係の保存方式を決定し、dependency line を描画する |
| 12 | 自動スケジューリング |
...
| Q-8 | 依存関係の保存先 | Projects v2 のテキストフィールドか、GitHub の sub-issue 関係か（§15.1） |

...
export function buildLinks(
  ...
  dependencies.forEach((dep, i) => {
    ...
    // 下限・上限を重ねて 2 段階（18→28→36 / 64→84）で上げてきた——「もっと
    // ゆとりが欲しい」という指摘のとおり、近接した接続に限らず全体を
    // ひと回り大きく、ふわっと弧を描くようにする。
    const bend = Math.max(36, Math.min(84, Math.max(Math.abs(x2 - x1) / 2, Math.abs(y2 - y1) / 2)))
...
export function buildMilestoneLinks(
  ...
  for (const link of links) {
    ...
    // 下限・上限を重ねて 2 段階（24→36→50 / 90→116）で上げてきた——buildLinks
    // と同じく、全体をひと回り大きく、ふわっと弧を描くようにする。
    const bend = Math.max(50, Math.min(116, Math.max(Math.abs(x2 - x1) / 2, Math.abs(y1 - y2) / 2)))
